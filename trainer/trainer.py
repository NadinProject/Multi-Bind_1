...
class Trainer():
    ...
    def train(self, train_loader: DataLoader, val_loader: DataLoader):
        ...
        for epoch in range(self.start_epoch, self.args.num_epochs + 1):  # loop over the dataset multiple times
            ...
            self.model.train()
            if self.epoch == 31:
                self.predict(train_loader, optim=self.optim, debug=True)
            else :
                self.predict(train_loader, optim=self.optim, debug=False)
            #self.predict(train_loader, optim=self.optim, debug=True)
            self.model.eval()
    ...
    def process_batch(self, batch, optim):
        loss, loss_components, _, targets = self.forward_pass(batch)
        if optim != None:  # run backpropagation if an optimizer is provided
            ...
            self.optim_steps += 1
        return loss, loss_components, None, list_detach(targets)

    def predict(self, data_loader: DataLoader, optim: torch.optim.Optimizer = None, return_pred=False, debug=False):
        ...
        for i, batch in enumerate(data_loader):
            ...
            *batch, batch_indices = move_to_device(list(batch), self.device)
            ...
            loss, loss_components, _, targets = self.process_batch(batch, optim)
            #binding_site=self.MultipleBindingSite(batch)
            with torch.no_grad():
